...
public class coding1_MorrisPreTraversal {
        ...
        while (cur != null) {  // 3）当cur为空的时候说明遍历结束
            if (cur.left != null) {  // 2）cur有左孩子
                ...
                if (mostRight.right == null) {  // 2）a.如果左子树的最右节点 的右指针指向空，让其指向cur，cur左移（这种情况下发生在当前节点是第一次到达）
                    ...
                    // 当然有的节点会到达1次，有的会到达2次。——有左孩子的节点会到达两次，没有左孩子的节点只到达一次。
                }
            } else {  // 这里必须用else，不能在代码"cur = cur.right;"上面直接加打印行为，因为"cur = cur.right;"不管if条件满不满足，都会被执行到
                System.out.print(cur.value + " ");
            }
